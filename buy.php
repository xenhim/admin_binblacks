...
{
    $price_in_btc = file_get_contents("https://blockchain.info/tobtc?currency=USD&value=1");
    if ($config['btcdeposit'] == '1' && $config['btcspeed'] != '1' || $config['UPMUSD'] == '1' || $config['UWMZ'] == '1' || $config['UPAYMERZ'] == '1')
    {
        ...
        $sql = "SELECT *,DATE_FORMAT(orderDate, '%d.%m.%Y') as sdate FROM " . $config['table_orders'] . " WHERE userId = '" . clean($_SESSION["userId"]) . "' AND (type = 'BTC' OR type = 'Unitaco PM' OR type = 'Unitaco WMZ' OR type = 'Unitaco PAYMER') ORDER BY orderId DESC";
