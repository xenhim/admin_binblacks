...
class XMPP
{
    ...
    function eventMessage($fromJid, $content, $offline = true)
    {
        // $content; текст сообщения
        // $fromJid от кого сообщение
        if ($offline) // офлайн сообщение
        {
            // можно как то обрабаотать входящее сообщение, а затем написать отправителю, что сообщение получено
            //$this->sendMessage($fromJid,"\nДанный jabber только для извещения пользователей!\n FOR-YOUR.BIZ"); // ответное сообщение
        }
        else // онлайн сообщение
        {
            // можно как то обрабаотать входящее сообщение, а затем написать отправителю, что сообщение получено
            //$this->sendMessage($fromJid,"\nДанный jabber только для извещения пользователей!\n FOR-YOUR.BIZ");  // ответное сообщение
        }
        ...
        {
            $this->sendMessage($fromJid, "\nДоступные команды:
                                                \nhelp - помощь."); // ответное сообщение
        }
    }
    ...
    */
    function __construct($webi_conf)
    {
        ...
        $this->lastid = 0;
        $this->lastSendTime = 0;
        $this->logEnable = $webi_conf['logtxt'];

        $this->sessionRequired = false;
        $this->secondChallenge = false;
        $this->presenceType = array(
            "unavailable",
            "subscribe",
            "subscribed",
            "unsubscribe",
        ...
        $this->logFileName = $webi_conf['log_file_name'];
        $this->host = $webi_conf['host'];
        $this->port = $webi_conf['port'];
        $this->user = $webi_conf['user'];
        $this->pass = $webi_conf['pass'];
        $this->domain = $webi_conf['domain'];
        $this->resource = "XServer";
        $this->status = "Active";
        $this->tls_off = $webi_conf['tls_off'];
        ...
        $this->logger("Initializing class variables");
    ...
    */
    function connect()
    {
        if (!$this->stream)
        {
            $this->logger("Trying to connect at " . $this->host . ":" . $this->port);
            if ($this->stream = @fsockopen($this->host, $this->port, $this->errorno, $this->errorstr, $this->timeout))
            {
                $this->logger("Connection made successfully at " . $this->host . ":" . $this->port);
            ...
            {
                $this->logger("Failed to establish a connection at " . $this->host . ":" . $this->port);
                return false;
            }
        }
        else
        {
            $this->logger("Already connected, sending start stream again at " . $this->host . ":" . $this->port);
            $this->isConnected = true;
            $this->sendStream();
            $this->getXML();
        }
    ...
    */
    ...
    {
        $xml = '<?xml version="1.0"?>';
        $xml .= '<stream:stream xmlns:stream="http://etherx.jabber.org/streams" version="1.0" xmlns="jabber:client" to="' . $this->
            ...
            if ($this->auth)
            {
                ...
                for ($i = 0; $i < $packetCount; $i++)
                {
                    ...
                    if ((!$this->xmlize->valid) && ($i == $packetCount - 1) && (substr($xmlarr[$i], 0, 14) != '<stream:stream'))
                    {
                        ...
                        $this->logger("Stored an invalid packet:\n" . $this->xmlBuffer);
                    }
                    else
                    {
                        if ($this->logEnable)
    ...
    */
    ...
    {
        if (isset($arr["stream:error"]["#"]["see-other-host"]) && $arr["stream:error"]["#"]["str:text"][0]["@"]["xmlns:str"] ==
    ...
    */
    ...
    {
        if ($arr["stream:stream"]['@']['xmlns'] != "jabber:client" || $arr["stream:stream"]['@']["xmlns:stream"] !=
        ...
        else
        {
            ...
            $xml = '';
            if (isset($arrStreamFeatures["#"]["starttls"]) and ($arrStreamFeatures["#"]["starttls"][0]["@"]["xmlns"] ==
                "urn:ietf:params:xml:ns:xmpp-tls") and !$this->tls_off)
                // если сервер поддерживает SSL и если шифрование не выключено принудительно
            {
                $this->logger("Starting TLS Encryption...");
                $xml = '<starttls xmlns="urn:ietf:params:xml:ns:xmpp-tls"/>';

            }
            else
                if (isset($arrStreamFeatures["#"]["mechanisms"]) && ($arrStreamFeatures["#"]["mechanisms"][0]["@"]["xmlns"] ==
                    "urn:ietf:params:xml:ns:xmpp-sasl"))
                {
                    ...
                    if (isset($arrStreamFeatures["#"]["bind"]) && ($arrStreamFeatures["#"]["bind"][0]["@"]["xmlns"] ==
                        "urn:ietf:params:xml:ns:xmpp-bind"))
                    {
                        $this->logger("Binding to the server stream");
    ...
    */
    ...
    {
        if ($arr["proceed"]["@"]["xmlns"] == "urn:ietf:params:xml:ns:xmpp-tls" and !$this->tls_off)
        {
    ...
    */
    function parseIq($arr)
    {
        if (isset($arr["iq"]["#"]["mailbox"]))
        {
            ...
            $this->eventNewEMail($GmailTotalMatched, $GmailThreadId, $GmailURL, $GmailParticipation, $GmailMessages, $GmailDate, $GmailSenders,
                $GmailLabels, $GmailSubject, $GmailSnippet);
        }
        else
            if (isset($arr["iq"]["#"]["new-mail"]) && $arr["iq"]["#"]["new-mail"][0]["@"]["xmlns"] == "google:mail:notify")
            {
                // Implementation of Google's Gmail Extension http://code.google.com/apis/talk/jep_extensions/gmail.html
                $this->getNewEMail();
            }
                ...
                if (isset($arr["iq"]["#"]["bind"]))
                {
                    ...
                    if (isset($arr["iq"]["#"]["query"]))
                    {
                        ...
                        if (isset($arr["iq"]["#"]["session"]) || ($arr['iq']['@']['type'] == "result" && $arr['iq']['@']['id'] == "sess_1"))
                        {
                            $this->bind();
                        }
    }
    ...
    */
    function sendStatus($status = null, $show = "chat", $priority = 5)
    {
        $xml = "<presence>";
        $xml .= "<show>" . $show . "</show>";
        if ($status)
        ...
        if (isset($arr["presence"]["@"]["type"]))
        {
            ...
            if (isset($arr["presence"]["#"]["status"]))
            {
                ...
                if (isset($arr["presence"]["#"]["x"][0]["#"]["photo"]))
                {
                    ...
                    if (isset($arr["presence"]["#"]["x"][1]["#"]["photo"]))
                    {
    ...
    */
    function sendPresence($type, $tojid)
    {
        ...
        {
            $this->logger("[[ERROR]]] Trying to send an inappropriate subscription type:" . $type);
            return false;
        }
    ...
    */
    ...
    {
        $arr = $arr["message"];
        if (isset($arr["@"]["type"]) && ($arr["@"]["type"] == "chat"))
        { //online messages
            $fromJid = $arr["@"]["from"];
            if (isset($arr["#"]["body"][0]["#"]))
            {
            ...
            if (isset($arr["#"]["x"][0]["@"]["xmlns"]) && ($arr["#"]["x"][0]["@"]["xmlns"] == "jabber:x:delay"))
            { //offline messages
                $fromJid = $arr["@"]["from"];
                $content = $arr["#"]["body"][0]["#"];
    ...
    function explodeData($data)
    {
        ...
        foreach ($data as $pair)
        {
            ...
            if ($dd)
            {
                ...
                if (strpos(strrev(trim($pair)), '"') === 0 && $key)
                {
